...
3. Из структуры `stat` берет ***НЕ*** поле `st_size` (байты) а поле `st_blocks`. Оно показывает, сколько блоков по 512 байт выделено [[Ядро операционной системы|ядром ОС]] на диске для этого файла.
